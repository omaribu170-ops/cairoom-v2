...
import {
    Play, Square, UserPlus, UserMinus, Search, Clock, Users, Eye,
    CreditCard, Wallet, Smartphone, Banknote, Calendar,
} from 'lucide-react';
...
const mockHistorySessions: HistorySession[] = [
    {
        id: 'hist-001', tableName: 'طاولة ١', date: '2024-12-28', startTime: '14:00', endTime: '17:30', members: [
            { id: 'm1', name: 'أحمد محمد', phone: '01012345678', joinedAt: '', leftAt: '', orders: [{ product: 'قهوة', quantity: 3, price: 25 }] },
        ], totalTimeCost: 88, totalOrdersCost: 75, grandTotal: 163
    },
    {
        id: 'hist-002', tableName: 'غرفة الاجتماعات', date: '2024-12-28', startTime: '10:00', endTime: '12:00', members: [
    ...
    {
        id: 'hist-003', tableName: 'طاولة ٣', date: '2024-12-27', startTime: '16:00', endTime: '19:00', members: [
            { id: 'm2', name: 'سارة أحمد', phone: '01123456789', joinedAt: '', leftAt: '', orders: [{ product: 'عصير', quantity: 2, price: 30 }] },
...
export default function SessionsPage() {
    ...
    const handleEndMemberSession = () => {
        ...
        toast.success(`تم إنهاء جلسة ${member.name} وحفظها في سجله`);
    ...
    const handleAddNewMember = () => {
        ...
        const newMember: SessionMember = {
            id: newId, name: newMemberName, phone: newMemberPhone,
            joinedAt: new Date().toISOString(), leftAt: null, orders: []
        };
        setActiveSessions(activeSessions.map(s => s.id === addMemberModal.id ? { ...s, members: [...s.members, newMember] } : s));
        toast.success(`تم إضافة ${newMemberName} للجلسة وقاعدة البيانات`);
        setNewMemberName('');
        setNewMemberPhone('');
        setShowAddNew(false);
    };
    ...
    const getSessionTotal = (session: ActiveSession) => {
        return session.members.filter(m => !m.leftAt).reduce((sum, m) => {
            const bill = getMemberBill(m, session.pricePerHour);
        ...
        <div className="space-y-6">
                                ...
                                <Card key={session.id} className="glass-card overflow-hidden">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground mt-1">
                                                    {session.members.filter(m => !m.leftAt).length} أعضاء نشطين
                                                </p>
                                            </div>
                                            <div className="text-left">
                                                <SessionTimer startTime={session.startTime} className="text-xl font-bold" />
                                                <p className="text-xs text-muted-foreground">وقت الجلسة</p>
                                            </div>
                                        </div>
                                    </CardHeader>
                                    <CardContent className="p-4 space-y-4">
                                            ...
                                            {session.members.filter(m => !m.leftAt).map((member) => {
                                                ...
                                                return (
                                                    <div key={member.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                                                        ...
                                                        <Button size="sm" variant="ghost" className="text-red-400 h-8" onClick={() => setEndMemberModal({ session, member })}>
                                        ...
                                        <div className="flex gap-2 pt-2">
                                            <Button size="sm" className="flex-1 gradient-button" onClick={() => setEndSessionModal(session)}>
                                                <Square className="h-4 w-4 ml-1" />إنهاء الجلسة
                                            </Button>
                                            <Button size="sm" variant="ghost" className="glass-button" onClick={() => setAddMemberModal(session)}>
                ...
                <TabsContent value="history" className="space-y-4 mt-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <Input placeholder="بحث بالاسم أو الهاتف أو رقم الجلسة..." value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)} className="glass-input pr-10" />
                        </div>
                            ...
                            <TableBody>
                                {filteredHistory.map((session) => (
                                    <TableRow key={session.id} className="border-white/5 hover:bg-white/5">
                                        ...
                                        <TableCell>
                                            <Button size="sm" variant="ghost" className="glass-button" onClick={() => setReceiptModal(session)}>
                        ...
                        <div className="space-y-4 py-4">
                            ...
                            <div className="glass-card p-4 space-y-3">
                                ...
                                <div className="border-t border-white/10 pt-2 flex items-center justify-between font-bold">
                                    <span>الإجمالي</span>
                                    <span className="gradient-text text-lg">{formatCurrency(getSessionTotal(endSessionModal))}</span>
                                </div>
                            </div>
                            {/* طريقة الدفع */}
                            <div className="space-y-2">
            ...
            <Dialog open={!!endMemberModal} onOpenChange={() => setEndMemberModal(null)}>
                <DialogContent className="glass-modal sm:max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="gradient-text text-xl">إنهاء جلسة العضو</DialogTitle>
                    </DialogHeader>
                    {endMemberModal && (() => {
                            ...
                            <div className="space-y-4 py-4">
                                <div className="text-center">
                                    <Avatar className="h-16 w-16 mx-auto mb-2">
                                        <AvatarFallback className="bg-gradient-to-br from-[#E63E32] to-[#F8C033] text-white text-xl">{endMemberModal.member.name.charAt(0)}</AvatarFallback>
                                    </Avatar>
                                ...
                                <div className="glass-card p-4 space-y-2 text-sm">
                                    ...
                                    <div className="border-t border-white/10 pt-2 flex justify-between font-bold">
                                        <span>الإجمالي</span><span className="gradient-text">{formatCurrency(bill.total)}</span>
                                    </div>
                                </div>
                ...
                <DialogContent className="glass-modal sm:max-w-md">
                    ...
                    <div className="space-y-4 py-4">
                        <div className="flex gap-2">
                            ...
                            <Button variant="ghost" className="glass-button" onClick={() => setShowAddNew(true)}>
                        ...
                        {memberSearch && (
                            <div className="glass-card p-2 space-y-1 max-h-40 overflow-y-auto">
                                {filteredMembers.map(member => (
                                    <button key={member.id} onClick={() => handleAddMemberToSession(member)} className="w-full p-3 text-right rounded-lg hover:bg-white/10 transition-colors">
                        ...
                        {showAddNew && (
                            <div className="glass-card p-4 space-y-3">
                                ...
                                <Button size="sm" className="w-full gradient-button" onClick={handleAddNewMember} disabled={!newMemberName.trim() || !newMemberPhone.trim()}>
                                    إضافة للجلسة وقاعدة البيانات
                                </Button>
                            </div>
                        )}
                    </div>
                    ...
                    {receiptModal && (
                        <div className="space-y-4 py-4">
                            <div className="glass-card p-4 space-y-2 text-sm">
                                ...
                                <div className="flex justify-between"><span className="text-muted-foreground">الطاولة</span><span>{receiptModal.tableName}</span></div>
